...
import { getWebSocketService, createWebSocketService, StreamingChatResponse } from '@/services/websocket'
...
export function useWebSocketStream(options: UseWebSocketStreamOptions = {}): UseWebSocketStreamReturn {
  ...
  const handleStreamingResponse = useCallback((response: StreamingChatResponse) => {
    ...
    if (response.message_id !== currentMessageIdRef.current) {
      // New message started
      currentMessageIdRef.current = response.message_id
      streamingTextRef.current = ''
    }
    
    // Accumulate text chunks
    if (response.text_chunk) {
      streamingTextRef.current += response.text_chunk
    }
    ...
    setIsStreaming(response.status === 'streaming')
  }, [])
  ...
  const handleStreamingComplete = useCallback((response: StreamingChatResponse) => {
    ...
    const finalMessage: StreamingMessage = {
      ...
      conversationId: response.conversation_id,
      role: 'assistant',
      content: response.text_complete || streamingTextRef.current,
      visualPayload: response.visual_payload,
      isStreaming: false,
      isComplete: true,
      metadata: response.metadata,
  ...
  const handleStreamingError = useCallback((response: StreamingChatResponse) => {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      if (!wsServiceRef.current) {
        const wsUrl = options.wsUrl || `${import.meta.env.VITE_API_BASE || 'http://localhost:8000'}/ws/chat`.replace('http', 'ws')
  ...
  }, [isConnecting, isConnected, options.wsUrl, options.maxReconnectAttempts, handleStreamingResponse, handleStreamingComplete, handleStreamingError])
  ...
  const sendMessage = useCallback((query: string, conversationId?: string, context?: any) => {
