...
import { TrendingUp, TrendingDown, Minus, MessageSquare, MoreVertical } from 'lucide-react'
  ...
  const handleAskSynrgy = () => {
    if (onAskSynrgy) {
      const context = `Tell me more about ${card.title.toLowerCase()}: currently ${card.value}${
        card.change
          ? `, ${card.change.trend} by ${Math.abs(card.change.value)}% ${card.change.period}`
          : ''
      }`
      onAskSynrgy(context)
    }
      ...
      >
        ...
        {card.change && (
          <div className={`flex items-center gap-2 text-sm ${getTrendColor()}`}>
            ...
            <span className="text-synrgy-muted">vs {card.change.period}</span>
          </div>
        )}
