...
export const SmartContextTrigger: React.FC<SmartContextTriggerProps> = ({
  ...
  const generateSmartQueries = (widget: DashboardWidget): Array<{
    intent: string
    query: string
  ...
  const renderTrigger = () => {
    switch (variant) {
      case 'button':
        return (
          <button className={`px-3 py-2 border border-synrgy-primary/20 rounded-lg text-sm font-medium text-synrgy-text hover:bg-synrgy-primary/10 hover:border-synrgy-primary/40 transition-all ${className}`}>
      ...
      case 'minimal':
        return (
          <button className={`p-1 h-7 w-7 rounded-lg text-synrgy-accent hover:bg-synrgy-accent/10 transition-colors ${className}`}>
            <MessageCircle className="h-3 w-3" />
          </button>
        )
      default:
        return (
          <button className={`p-2 rounded-lg text-synrgy-muted hover:text-synrgy-primary hover:bg-synrgy-primary/10 transition-colors ${className}`}>
  ...
  return (
    <div className="relative">
      <div
        title="Spawn focused chat session for this widget"
                        ...
                        >
                          <div className="flex items-start gap-2">
                            ...
                            <div className="flex-1">
                              ...
                              <div className={`inline-block mt-2 px-2 py-0.5 text-xs border rounded-full ${getCategoryColor(suggestion.category)}`}>
                                {suggestion.category}
                              </div>
                            </div>
                          </div>
