/**
 * Main Application Store for SYNRGY
 * Handles auth, UI state, system health, and notifications
 */

import { create } from 'zustand'
import { persist, devtools } from 'zustand/middleware'
import { authApi, systemApi, adminApi } from '@/services/api'
import type { User, SiemConnector, UIState } from '@/types'
...
const createAppStore = () => {
  const store = (set: any, get: any) => ({
    ...
    toggleSidebar: () => {
      set((state: AppState) => ({ sidebarCollapsed: !state.sidebarCollapsed }))
    },
    ...
    toggleChatPanel: () => {
      set((state: AppState) => ({ chatPanelOpen: !state.chatPanelOpen }))
    },
  ...
  // Return store with devtools only in development
  if (import.meta.env.VITE_ENABLE_DEVTOOLS === 'true') {
    return devtools(persist(store, persistConfig), { name: 'SYNRGY App Store' })
  } else {
    return persist(store, persistConfig)
  }
}
