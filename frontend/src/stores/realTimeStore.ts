  ...
  persist(
    (set, get) => ({
      ...
      addUpdate: (update: Omit<RealTimeUpdate, 'id' | 'timestamp'>) => {
        ...
        // Add to queue
        const newQueue = [...state.updateQueue, newUpdate]
          .slice(-state.config.maxUpdateQueueSize)
        
        // Add to history
        const newHistory = [...state.updateHistory, newUpdate]
      ...
      processUpdateQueue: async () => {
        ...
        try {
          ...
          const updatesByWidget = updates.reduce((acc, update) => {
      ...
      recordUpdate: (success: boolean, latency = 0) => {
        ...
        set({
          metrics: {
            ...
            failedUpdates: success 
              ? state.metrics.failedUpdates 
              : state.metrics.failedUpdates + 1
          }
        })
      },
      ...
      optimizeSubscriptions: () => {
        ...
        set({
          subscriptions: state.subscriptions.filter(s => 
            s.isActive || s.lastUpdate > fiveMinutesAgo
          )
        })
      },
