...
interface HybridModeState {
  ...
  startFocusedSession: (type: 'widget' | 'chat' | 'dashboard', id: string, context: ConversationContext, spawnQuery?: string) => void
  endFocusedSession: () => void
  updateFocusedSession: (context: ConversationContext) => void
  
  // Context history management
  addContextHistoryEntry: (entry: Omit<HybridModeState['contextHistory'][0], 'id' | 'timestamp'>) => void
  ...
  findOptimalPosition: (widget?: DashboardWidget) => { row: number, col: number, width: number, height: number }
  optimizeLayout: () => void
}
...
export const useHybridStore = create<HybridModeState>()(
  persist(
    (set, get) => ({
      ...
      updateWidgetPosition: (widgetId: string, position: { x: number, y: number, w: number, h: number }) => {
        const state = get()
        set({
          widgetLayout: {
            ...state.widgetLayout,
      ...
      syncContexts: () => {
        ...
        const mergedContext = {
          conversation_id: chatContext?.conversation_id || dashboardContext?.conversation_id || focusedContext?.conversation_id || 'hybrid_session',
          history: [
          ...
          time_range: focusedContext?.time_range || chatContext?.time_range || dashboardContext?.time_range,
          metadata: {
      ...
      // Focused session management
      startFocusedSession: (type: 'widget' | 'chat' | 'dashboard', id: string, context: ConversationContext, spawnQuery?: string) => {
      ...
      endFocusedSession: () => {
        ...
        if (state.focusedSession) {
          ...
          get().addContextHistoryEntry({
            type: 'sync',
            from: state.focusedSession.type === 'widget' ? 'dashboard' : state.focusedSession.type as 'dashboard' | 'chat',
            to: state.focusedSession.type === 'widget' ? 'chat' : state.focusedSession.type as 'dashboard' | 'chat',
      ...
      // Context history management
      addContextHistoryEntry: (entry: Omit<HybridModeState['contextHistory'][0], 'id' | 'timestamp'>) => {
        const state = get()
        const historyEntry = {
          ...entry,
        ...
        const updatedHistory = [...state.contextHistory, historyEntry]
      ...
      getContextHistoryForEntity: (entityId: string): HybridModeState['contextHistory'] => {
        const state = get()
        return state.contextHistory.filter(entry => 
          entry.context.entities && Object.keys(entry.context.entities).includes(entityId)
        )
      },
      ...
      generateWidgetFromPayload: (payload: VisualPayload, messageId: string): DashboardWidget => {
        ...
        if (payload.type === 'composite' && payload.cards) {
          ...
          return {
            id: widgetId,
            type: payload.type === 'chart' ? 'chart' : 
                  payload.type === 'table' ? 'table' :
                  payload.type === 'narrative' ? 'insight_feed' : 'summary_card',
